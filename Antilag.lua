...
local function CheckAllDescendants(Instance)
...
local function UpdateLighting()
    ...
    if _G.Settings.Other["Low Rendering"] then
        Lighting.Quality = Enum.QualityLevel.Low
    end
...
local function OnCharacterAdded(Character)
    Character.DescendantAdded:Connect(CheckIfBad)
end

ME.CharacterAdded:Connect(OnCharacterAdded)

-- Initialize settings and check for existing objects
Init()

-- Check existing objects in the StarterPlayer and other places
for _, Instance in pairs(StarterGui:GetDescendants()) do
    CheckIfBad(Instance)
end
...
-- Optionally adjust the FPS cap
if _G.Settings.Other["FPS Cap"] then
    game:GetService("RunService"):Set3dRenderingSettings({
        MaxFramesPerSecond = _G.Settings.Other["FPS Cap"]
    })
end

-- Run an update loop to continually check for new instances
